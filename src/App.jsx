...
const s = {
  app: { minHeight: "100dvh", background: "#0f0f0f", color: "#e8e6e1", fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column" },
  header: { padding: "16px 20px 0", borderBottom: "1px solid #1e1e1e", background: "#0f0f0f", position: "sticky", top: 0, zIndex: 10 },
  body: { padding: "20px", flex: 1, overflowY: "auto", paddingBottom: "32px" },
  card: { background: "#161616", border: "1px solid #262626", borderRadius: 16, padding: "16px", marginBottom: 12, cursor: "pointer", transition: "border-color 0.15s" },
  statCard: { background: "#161616", border: "1px solid #262626", borderRadius: 12, padding: "14px", flex: 1, textAlign: "center" },
  btn: { width: "100%", padding: "14px", borderRadius: 12, fontSize: 15, fontWeight: 500, cursor: "pointer", border: "none", marginBottom: 10, fontFamily: "inherit" },
  ...
  tag: { display: "inline-block", fontSize: 11, fontWeight: 500, padding: "3px 9px", borderRadius: 99, marginRight: 6, marginTop: 6 },
  textarea: { width: "100%", background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 12, padding: 14, color: "#e8e6e1", fontSize: 15, lineHeight: 1.6, resize: "none", fontFamily: "inherit", outline: "none", boxSizing: "border-box" },
  label: { fontSize: 11, color: "#555", fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 6, display: "block" },
  dimBar: { flex: 1, height: 5, background: "#1e1e1e", borderRadius: 4, margin: "0 10px", overflow: "hidden" },
  filterBtn: (active) => ({ padding: "6px 14px", borderRadius: 99, fontSize: 12, fontWeight: 500, border: active ? "1px solid #10b981" : "1px solid #262626", background: active ? "#052e16" : "#161616", color: active ? "#10b981" : "#666", cursor: "pointer", whiteSpace: "nowrap" }),
  backBtn: { background: "#1e1e1e", border: "1px solid #262626", borderRadius: 8, padding: "6px 12px", color: "#9ca3af", cursor: "pointer", fontSize: 13, fontFamily: "inherit" },
};

const ScoreRing = ({ score, size = 64 }) => {
  const c = scoreColor(score);
  return (
    <div style={{ width: size, height: size, borderRadius: "50%", border: `2.5px solid ${c.ring}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0, background: "#161616" }}>
...
const BarDim = ({ label, value }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #1e1e1e" }}>
      ...
      <div style={s.dimBar}><div style={{ height: "100%", width: `${value * 10}%`, background: c.ring, borderRadius: 4 }} /></div>
...
const EstadoTag = ({ estado }) => {
  const m = { pendiente: ["#facc15", "#1c1700", "Pendiente"], en_progreso: ["#10b981", "#052e16", "En progreso"], descartada: ["#ef4444", "#1c0707", "Descartada"] };
...
export default function App() {
  ...
  // Cargar ideas desde Supabase al iniciar
  useEffect(() => {
    cargarIdeas();
  }, []);
  ...
  const guardarIdea = async () => {
    ...
    const { data, error } = await supabase.from("ideas").insert([nueva]).select();
    if (!error && data) {
  ...
  const cambiarEstado = async (idea, nuevoEstado) => {
    const { error } = await supabase.from("ideas").update({ estado: nuevoEstado }).eq("id", idea.id);
    if (!error) {
      setIdeas((prev) => prev.map((i) => i.id === idea.id ? { ...i, estado: nuevoEstado } : i));
      setIdeaDetalle((prev) => prev ? { ...prev, estado: nuevoEstado } : prev);
    }
  };
  ...
  const ideasFiltradas = ideas.filter((i) => filtro === "todas" ? true : i.estado === filtro);
      ...
      <div style={s.header}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 16 }}>
          <div>
            ...
            <h1 style={{ fontSize: 24, fontWeight: 600, margin: 0 }}>Mis ideas</h1>
          </div>
          <button onClick={() => setScreen("nueva")} style={{ background: "#10b981", color: "#fff", border: "none", borderRadius: 10, padding: "9px 18px", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>+ Nueva</button>
        </div>
      ...
      <div style={s.body}>
        <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
          <div style={s.statCard}>
            <p style={{ fontSize: 26, fontWeight: 600, margin: "0 0 2px" }}>{ideas.length}</p>
            <p style={{ fontSize: 11, color: "#555", margin: 0 }}>guardadas</p>
          </div>
          <div style={s.statCard}>
            <p style={{ fontSize: 26, fontWeight: 600, margin: "0 0 2px", color: "#10b981" }}>{ideas.filter((i) => i.estado === "en_progreso").length}</p>
            <p style={{ fontSize: 11, color: "#555", margin: 0 }}>en progreso</p>
          </div>
          <div style={s.statCard}>
            <p style={{ fontSize: 26, fontWeight: 600, margin: "0 0 2px", color: "#f59e0b" }}>
              {ideas.length > 0 ? (ideas.reduce((a, b) => a + (b.score_total || 0), 0) / ideas.length).toFixed(1) : "—"}
            </p>
        ...
        {loadingIdeas && (
          <div style={{ textAlign: "center", padding: "40px 0", color: "#555" }}>
            <p style={{ fontSize: 14 }}>Cargando ideas...</p>
          </div>
        )}

        {!loadingIdeas && ideasFiltradas.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 0", color: "#333" }}>
            <p style={{ fontSize: 32, marginBottom: 8 }}>💡</p>
            <p style={{ fontSize: 15 }}>No hay ideas aquí todavía</p>
            <p style={{ fontSize: 13, color: "#555", marginTop: 4 }}>Presioná "+ Nueva" para empezar</p>
          </div>
        )}

        {[...ideasFiltradas].sort((a, b) => (b.score_total || 0) - (a.score_total || 0)).map((idea) => (
          <div key={idea.id} style={s.card}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 14, fontWeight: 500, margin: "0 0 4px", lineHeight: 1.4 }}>
                  {idea.texto.length > 80 ? idea.texto.slice(0, 80) + "…" : idea.texto}
                </p>
                <p style={{ fontSize: 12, color: "#555", margin: 0 }}>{idea.fecha}</p>
              </div>
              <ScoreRing score={idea.score_total} size={52} />
            </div>
            <div style={{ marginTop: 8 }}>
              <EstadoTag estado={idea.estado} />
              <span style={{ ...s.tag, background: "#0d1f16", color: "#10b981" }}>{idea.nivel}</span>
            </div>
          </div>
        ))}
  ...
  if (screen === "nueva") return (
    <div style={s.app}>
      ...
      <div style={s.body}>
        ...
        <textarea style={s.textarea} rows={7}
          placeholder="Escribí tu idea con tus propias palabras. Ej: Una app que permite a agencias de viaje generar cotizaciones en segundos..."
        ...
        <button onClick={analizar} disabled={loading || !texto.trim()}
      ...
      <div style={s.app}>
        ...
        <div style={s.body}>
          <div style={{ background: "#161616", border: `1px solid ${c.ring}33`, borderRadius: 16, padding: 16, marginBottom: 16, display: "flex", gap: 16, alignItems: "center" }}>
            ...
            <div>
              <p style={{ fontSize: 15, fontWeight: 600, margin: "0 0 4px" }}>{a.nivel}</p>
          ...
          <div style={{ background: "#161616", border: "1px solid #262626", borderRadius: 16, padding: 16, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", paddingTop: 10 }}>
              ...
              <div style={s.dimBar}><div style={{ height: "100%", width: `${a.score_ejecutable * 10}%`, background: scoreColor(a.score_ejecutable).ring, borderRadius: 4 }} /></div>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
            {[["tiempo_lanzamiento","Para lanzar","#e8e6e1"],["precio_estimado","Precio","#10b981"],["clientes_meta","Clientes meta","#f59e0b"]].map(([k,l,col]) => (
              <div key={k} style={{ ...s.statCard, textAlign: "center" }}>
                <p style={{ fontSize: 14, fontWeight: 600, margin: "0 0 2px", color: col }}>{a[k]}</p>
                <p style={{ fontSize: 11, color: "#555", margin: 0 }}>{l}</p>
              </div>
            ))}
          </div>
  ...
  if (screen === "detalle" && ideaDetalle) {
    ...
    return (
      <div style={s.app}>
        ...
        <div style={s.body}>
          <p style={{ fontSize: 15, color: "#e8e6e1", lineHeight: 1.6, marginBottom: 12 }}>{a.texto}</p>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
            <EstadoTag estado={a.estado} />
            <span style={{ ...s.tag, background: "#0d1f16", color: "#10b981" }}>{a.nivel}</span>
            <span style={{ fontSize: 12, color: "#555" }}>{a.fecha}</span>
          </div>
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {["pendiente", "en_progreso", "descartada"].map((e) => (
              <button key={e} onClick={() => cambiarEstado(a, e)} style={{ flex: 1, padding: "8px 4px", borderRadius: 8, fontSize: 11, fontWeight: 500, cursor: "pointer", fontFamily: "inherit", background: a.estado === e ? "#0d1a14" : "#161616", border: a.estado === e ? "1px solid #10b981" : "1px solid #262626", color: a.estado === e ? "#10b981" : "#555" }}>
          ...
          <button onClick={() => eliminarIdea(a.id)} style={{ ...s.btn, ...s.btnDanger }}>Eliminar idea</button>
        </div>
      </div>
    );
  }
